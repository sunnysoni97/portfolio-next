...
const WorkExEntry = ({ listItem }) => {
	return (
		<div>
			...
			<Text className="!text-sm !font-bold opacity-70" my="0.5em">{`${
				listItem.start_month ? GetMonthStr(listItem.start_month) : ""
			} ${listItem.start_year} - ${
				listItem.end_month ? GetMonthStr(listItem.end_month) : ""
			} ${listItem.end_year}`}</Text>
			<br />
			<Text className="text-justify">{listItem.desc}</Text>
			<br />
			<Text className="text-left italic font-bold !text-sm">
				{"|"}
				{listItem.skills.map((value, _) => {
					return ` ${value.trim()} | `;
				})}
			</Text>
